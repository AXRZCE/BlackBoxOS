...
export const projects: Project[] = [
  ...
  },
  {
    ...
    context: 'A simulation-calibrated prediction market system that tests whether AI-driven forecasts can outperform real-money prediction markets. Built a full validation pipeline from market scanning to calibrated scoring — and proved that markets win on short-deadline hard-data events.',
    sections: {
      problem: 'Prediction markets are efficient aggregators of information. The question: can AI simulation-based forecasting beat them? Needed a rigorous calibration framework to test this before risking any real capital.',
      constraints: [
      ...
      decisions: [
        'MiroFish simulation engine with DeepSeek V4 Pro for multi-run ensemble forecasts',
        'Brier score as primary calibration metric with false-confidence diagnostics',
        'Structured phase gates: Track A (observation) → Track C (validation) → Phase 2 (locked)',
        'High-divergence guardrails: >0.30 gap reruns, >0.50 excludes from scoring',
        'English-language enforcement across all 4 MiroFish prompt paths',
      ],
      outcome: '22 Track C events resolved across Kalshi and Polymarket. Market Brier scores decisively beat simulation scores — proving the calibration framework works and preventing premature trading. 13/13 parser success rate, zero market-price-copy failures.',
      learnings: [
        'Markets crush LLMs on short-deadline hard-data events — this was the most important finding',
        'Single-run agent simulations are unstable; ensemble runs (--runs N) dramatically improve reliability',
        'Seed quality controls (current-price context, fact/speculation separation) matter as much as the model',
        'Phase gates with scored-evidence requirements prevent emotionally-driven trading decisions',
        'Track separation (A/B/C) is essential: Brier scores must never be blended across tracks',
      ],
    },
    problem: 'Can AI simulation-based forecasting beat prediction markets? Needed a rigorous calibration framework before risking capital.',
    ...
    results: '22 events resolved. Market forecasts beat simulation forecasts across scored Track C set. 13/13 parser success. Framework proved itself by preventing premature trading.',
    artifacts: [
    ...
    color: '#f59e0b',
    outcomes: [
      'Built end-to-end calibration pipeline: scan → seed → simulate → parse → score → report',
      'Proved market efficiency on short-deadline events with 22 resolved Track C markets',
      'Implemented ensemble simulation support with high-divergence guardrails',
      'Phase-gated architecture prevents trading before scored evidence of edge',
    ],
    metrics: [
    ...
    highlights: [
      'Built rigorous calibration framework proving markets beat LLMs on hard-data short-deadline events',
      'Implemented ensemble simulation with divergence guardrails to catch unstable forecasts',
      'Designed phase-gated architecture requiring scored evidence before any trading decision',
    ],
  },
  {
    id: 'axrzce-fund',
    ...
    context: 'A multi-agent financial research system where specialized AI agents handle market research, fundamental analysis, risk assessment, portfolio allocation, and strategy debate — mirroring the workflow of an institutional investment committee.',
    sections: {
      problem: 'Most AI trading tools are single-model black boxes. Real investment decisions come from debate and reconciliation across specialists — research, fundamentals, risk, strategy. No existing open-source system modeled this multi-agent committee structure.',
      constraints: [
      ...
      decisions: [
        'LangGraph for agent orchestration with directed debate graph topology',
        'CrewAI for role-based agent specialization and tool delegation',
        'Debate protocol: Strategy and Analysis agents reconcile conflicting signals before output',
        'FastAPI + PostgreSQL for persistence and serving',
      ],
      outcome: 'Functional multi-agent research system producing auditable trade recommendations. Specialized agents debate and reconcile signals before output. Mirrors institutional investment committee structure.',
      learnings: [
        ...
        'Role specialization (separate Fundamental vs Technical agents) produces richer analysis',
        'Debate protocols need explicit reconciliation rules — otherwise agents talk past each other',
      ],
    },
    problem: 'Most AI trading tools are single-model black boxes. Real investment decisions come from debate across specialists.',
    ...
    highlights: [
      ...
      'Implemented auditable decision trails for full research-to-recommendation traceability',
    ],
  },
  {
    id: 'compi',
